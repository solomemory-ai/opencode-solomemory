...
| `messages.ts`                      | 57  | `MessageInfo`/`SessionMessage` types, `extractContentFromParts()` (text-only), `extractReasoningText()` (reasoning), `extractToolEntries()` (completed ToolParts → compact tool entries)                        |
